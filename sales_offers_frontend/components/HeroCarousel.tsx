          ...
          <div className="relative w-full h-full flex items-center justify-center" style={{ perspective: '2000px' }}>
            ...
            >
              {items.map((item, index) => (
                <div
                  key={item.id}
                  className={`carousel-slide absolute rounded-3xl overflow-hidden shadow-2xl bg-white/5 border border-white/20 cursor-pointer transition-all duration-700 ${index === currentIndex ? 'active brightness-100' : 'brightness-60'
                    }`}
                  style={{
                    width: '100%',
                    height: '100%'
                ...
                >
                  ...
                  {/* Desktop CTA Overlay */}
                  <div className={`absolute bottom-8 left-1/2 transform -translate-x-1/2 transition-all duration-600 z-30 ${index === currentIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
                    } hidden lg:block`}>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <div className={`lg:hidden absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/95 via-black/70 to-transparent text-white transition-all duration-500 ${index === currentIndex ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                    }`}>
